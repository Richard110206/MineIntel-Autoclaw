...
description: >
  ...
  MineIntel 实时进度网页展示，以及 HTML 完整报告和文献综述 LaTeX/PDF 生成。用户提到
...
- `mineintel-html-poster`：MineIntel magazine-poster 风格 HTML 完整报告。
...
5. 最终输出保存到 `output/<时间戳_报告标题>/`，至少包含 HTML 完整报告、文献综述 `.tex`，如果工作区内存在 `local_tools/MiKTeX/.../xelatex.exe` 则编译 PDF。
